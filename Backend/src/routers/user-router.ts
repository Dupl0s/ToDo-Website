...
export const buildUserRouter = (userRepo: UserRepository) => {
    ...
    router.post("/", (req: Request, res: Response) => {
        ...
        if (success) {
            ...
            res.status(201).json({ message: "User wurde erstellt", username: newUser.username, email: newUser.email, userId: newUser.userId });
        }
        if (!success) {
            res.status(400).json({
                message: "Error: " + error,
    ...
    router.get("/", (req: Request, res: Response) => {
        ...
        if (success) {
            ...
            else res.status(404).json({ message: "Kein Benutzer mit dieser Mailadresse vorhanden" })
        }
        if (!success) {
    ...
    router.post("/login", (req: Request, res: Response) => {
        ...
        if (success) {
            ...
            if (user) {
                ...
                res.status(200).json({ message: "Benutzer vorhanden und verifiziert" , 
